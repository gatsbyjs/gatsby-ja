...
Google 翻訳など、機械翻訳サービスを利用して得られた文章をほぼそのまま流用することは推奨していません。機械翻訳の結果は一見すると文法構造的に正しく見えますが、実際には文脈的に誤った翻訳になっていることが多いためです。また、サービスによっては私的利用以外を禁止しているものがあります。そのようなサービスを使った翻訳だと知らないでマージしてしまい、後でチームがそれに気付いた場合、過去のコミットを全て精査した上で再翻訳する必要があります。
