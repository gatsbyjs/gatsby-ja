...
Netlify は Gatsby サイトをデプロイするための素晴らしい手段です。Netlify はあなたのコードを簡単にメンテナンスでき、かつ性能も良いウェブサイトもしくはウェブアプリとして公開するための統合された自動化プラットフォームで、Git をトリガーとした継続的デプロイをはじめ、インテリジェントなグローバル CDN 、DNS 機能、自動 HTTPS 化、コンテンツの高速配信など多くの機能を提供します。

無料版には無制限の個人・商業プロジェクトと HTTPS 化、公開・非公開リポジトリーからの継続的デプロイなどが含まれます。
