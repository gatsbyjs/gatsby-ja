...
あなたのサイト上でユーザーが検索できるようにしたい任意の場所から呼び出す React の `Search` コンポーネントにあなたが必要なものをすべて集めます。デザインはサイトごとに大きく異なりますが、検索フィールドをユーザーがクリックした際にスライドアウトさせるための CSS トランジションや Algoria が返す一致結果を表示するための結果ペインについて前もって考えるのは時間がかかるので、このガイドの [`styled-components`](https://styled-components.com) を使用した実装が参考になるでしょう。
