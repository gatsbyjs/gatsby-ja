...
もし [チュートリアルの Part 0](/tutorial/part-zero/#install-nodejs-for-your-appropriate-operating-system) にしたがっていれば、すでに Homebrew をインストールしているはずです。Homebrew は Node.js（およびその他のソフトウェア）の特定のバージョンをインストールできるプログラムです。
