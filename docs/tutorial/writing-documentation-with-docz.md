...
Docz は mdx ファイル（Markdown for JSX の略）を活用して、 **React コンポーネント**を Markdown に記述できます。また、 PropTypes 、または Flow types または TypeScript にも対応しています。これらの **React コンポーネント**から**コンポーネントの持つプロパティ一覧**を生成して、コンポーネントの使用方法を適切に文書化できます。さらに、**コンポーネントのサンプル実行環境**を提供して、だれでもコンポーネントの動作を確認したり、コードを変更して変更をライブで確認したり、コードをコピーして別の場所で使用したりできます。
...
インストール後に、 `gatsby-config.js` ファイルの `plugins` 配下に `gatsby-theme-docz` を追加します。
...
ボタンは、一般的なアクションをより明確にし、ユーザーがより簡単に実行できるようにします。ボタンはラベルとアイコンを使用して、ユーザーがボタンに触れたときに発生するアクションを伝えます。
```

Docz では、コンポーネントとそのプロパティを表示するための内部コンポーネントをいくつか用意しています。これらとドキュメント化するコンポーネント自体の両方をドキュメントにインポートして使用します。
