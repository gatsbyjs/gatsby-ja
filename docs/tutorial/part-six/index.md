...
多くの場合、ソースプラグインで取得したデータ形式は Web サイトの構築に使用されるものではありません。ファイルシステムソースプラグインを使用すればファイルのメタデータを取得できますが、ファイル内に入力されているデータは取得できません。
...
Gatsby はビルド時に GraphQL を使用してデータを取得し、クエリのすべての結果をページへマッピングします。
